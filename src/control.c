/**
 * @file control.c
 * @brief Flight Control Output Implementation for ESP32-C3 Flight Controller
 *
 * Refactored to match include/control.h / include/config.h.
 * - SG90/servo: outputs via LEDC PWM (50Hz)
 * - DSHOT300/600: SAFE STUB (does not generate DSHOT waveform). Throttle is cut.
 */

#include "control.h"
#include "config.h"
#include "xm_plus.h"

#include <string.h>

#include "driver/ledc.h"
#include "driver/gpio.h"
#include "esp_log.h"

#include "freertos/FreeRTOS.h"
#include "freertos/semphr.h"

static const char *TAG = "control";

// Static state for control system
static control_outputs_t s_outputs = {0};
static SemaphoreHandle_t s_control_mutex = NULL;
static bool s_armed = true;
static bool s_failsafe_active = false;
static bool s_initialized = false;

// Selected output type (runtime via control_set_output_type)
static output_type_t s_output_type = OUTPUT_TYPE_SERVO_SG90;

// LEDC timer/channel assignments
// We map:
// - pwm_ch1_us -> PIN_PWM_1
// - pwm_ch2_us -> PIN_PWM_2
// - pwm_ch3_us -> PIN_PWM_3
// - pwm_ch4_us -> PIN_PWM_4
// - pwm_ch5_us -> PIN_PWM_5 (optional)
// - pwm_ch6_us -> PIN_PWM_6 (optional)
//
// LEDC channels used:
// - 0..5 for pwm_ch1..pwm_ch6
// LEDC channel indexes (must match the mapping to PIN_PWM_1..PIN_PWM_6)
#define PWM_CH_1 0
#define PWM_CH_2 1
#define PWM_CH_3 2
#define PWM_CH_4 3
#define PWM_CH_5 4
#define PWM_CH_6 5


static void set_pwm_duty(ledc_channel_t ch, uint32_t duty)
{
    ledc_set_duty(LEDC_LOW_SPEED_MODE, ch, duty);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, ch);
}


static uint32_t clamp_u32(uint32_t v, uint32_t lo, uint32_t hi)
{
    if (v < lo) return lo;
    if (v > hi) return hi;
    return v;
}

static uint16_t clamp_u16(uint16_t v, uint16_t lo, uint16_t hi)
{
    if (v < lo) return lo;
    if (v > hi) return hi;
    return v;
}

// SBUS (172..1811) -> 0..1000
static uint16_t sbus_to_0_1000(uint16_t sbus_value)
{
    const uint16_t sbus_min = 172;
    const uint16_t sbus_max = 1811;
    sbus_value = clamp_u16(sbus_value, sbus_min, sbus_max);
    return (uint16_t)(((uint32_t)(sbus_value - sbus_min) * 1000U) / (uint32_t)(sbus_max - sbus_min));
}

// Map servo pulse us [SG90_PWM_MIN_US..SG90_PWM_MAX_US] from control units [0..1000] where center=500
// We interpret:
// - input 0 -> SG90_PWM_MIN_US
// - input 1000 -> SG90_PWM_MAX_US
static uint16_t ctrl_to_servo_us(uint16_t ctrl_0_1000)
{
    // ctrl_0_1000 clamped to 0..1000
    uint16_t c = clamp_u16(ctrl_0_1000, 0, 1000);
    return (uint16_t)(((uint32_t)c * (SG90_PWM_MAX_US - SG90_PWM_MIN_US)) / 1000U + SG90_PWM_MIN_US);
}

static uint32_t servo_us_to_duty(uint16_t us)
{
    // LEDC timer is configured for SG90_PWM_FREQ_HZ and PWM_RESOLUTION_BITS.
    const uint32_t period_us = 1000000U / SG90_PWM_FREQ_HZ;
    const uint32_t duty = ((uint32_t)us * ((1U << SG90_PWM_RESOLUTION_BITS) - 1U)) / period_us;
    const uint32_t max_duty = (1U << SG90_PWM_RESOLUTION_BITS) - 1U;
    return clamp_u32(duty, 0, max_duty);
}

static void apply_outputs_locked(void)
{
    // Always drive servo channels (SG90 PWM) if output type is servo.
    // For DSHOT stubs: we still output servos for attitude channels, but we CUT throttle.

    // CH1..CH4 mapping:
    // In your original control logic:
    // - CH3 throttle/motor
    // - CH1 roll, CH2 pitch, CH4 yaw
    // With struct naming pwm_ch1_us..pwm_ch4_us:
    // We'll keep semantic as:
    // - pwm_ch1_us = roll
    // - pwm_ch2_us = pitch
    // - pwm_ch3_us = throttle
    // - pwm_ch4_us = yaw

    uint16_t roll_ctrl = clamp_u16(s_outputs.pwm_ch2_us, 0, 1000);  // roll is stored in pwm_ch2_us in our mapping below
    // To avoid confusion, we will recompute duties from stored struct fields:
    // pwm_ch1_us: throttle? pwm_ch2_us roll? etc.
    // However control_outputs_t comments are generic; we define mapping in update_from_sbus.

    // In apply stage, we interpret according to update_from_sbus:
    // pwm_ch1_us = throttle (0..1000)
    // pwm_ch2_us = roll (0..1000)
    // pwm_ch3_us = pitch (0..1000)
    // pwm_ch4_us = yaw (0..1000)

    // If disarmed OR failsafe OR output type is DSHOT => cut throttle.
    bool cut_throttle = (!s_armed) || s_failsafe_active || (s_output_type == OUTPUT_TYPE_DSHOT300) || (s_output_type == OUTPUT_TYPE_DSHOT600);
    uint16_t throttle_ctrl = cut_throttle ? 0 : clamp_u16(s_outputs.pwm_ch1_us, 0, 1000);

    uint16_t roll_ctrl_u   = clamp_u16(s_outputs.pwm_ch2_us, 0, 1000);
    uint16_t pitch_ctrl_u  = clamp_u16(s_outputs.pwm_ch3_us, 0, 1000);
    uint16_t yaw_ctrl_u    = clamp_u16(s_outputs.pwm_ch4_us, 0, 1000);

    // Convert to servo pulse widths
    uint16_t throttle_us = ctrl_to_servo_us(throttle_ctrl);
    uint16_t roll_us     = ctrl_to_servo_us(roll_ctrl_u);
    uint16_t pitch_us    = ctrl_to_servo_us(pitch_ctrl_u);
    uint16_t yaw_us      = ctrl_to_servo_us(yaw_ctrl_u);

    // Set LEDC duties
    // pwm_ch1_us (throttle) -> LEDC_CHANNEL_PWM1 -> PIN_PWM_1
    set_pwm_duty((ledc_channel_t)PWM_CH_1, servo_us_to_duty(throttle_us));
    // roll -> pwm2
    set_pwm_duty((ledc_channel_t)PWM_CH_2, servo_us_to_duty(roll_us));
    // pitch -> pwm3
    set_pwm_duty((ledc_channel_t)PWM_CH_3, servo_us_to_duty(pitch_us));
    // yaw -> pwm4
    set_pwm_duty((ledc_channel_t)PWM_CH_4, servo_us_to_duty(yaw_us));

    // AUX PWM channels (optional)
    set_pwm_duty((ledc_channel_t)PWM_CH_5, servo_us_to_duty(ctrl_to_servo_us(clamp_u16(s_outputs.pwm_ch5_us, 0, 1000))));
    set_pwm_duty((ledc_channel_t)PWM_CH_6, servo_us_to_duty(ctrl_to_servo_us(clamp_u16(s_outputs.pwm_ch6_us, 0, 1000))));


    // Digital outputs (LEDs): dig_ch1_state/dig_ch2_state -> PIN_OUTPUT_1/2
    gpio_set_level(PIN_OUTPUT_1, s_outputs.dig_ch1_state ? 1 : 0);
    gpio_set_level(PIN_OUTPUT_2, s_outputs.dig_ch2_state ? 1 : 0);
}

static void apply_outputs(void)
{
    if (s_control_mutex == NULL) return;
    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;
    apply_outputs_locked();
    xSemaphoreGive(s_control_mutex);
}

bool control_set_output_type(output_type_t type)
{
    if (type == OUTPUT_TYPE_NONE) return false;

    if (s_control_mutex != NULL && xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        s_output_type = type;
        xSemaphoreGive(s_control_mutex);
        return true;
    }

    // If not initialized yet, just set.
    s_output_type = type;
    return true;
}

bool control_init(void)
{
    if (s_initialized) {
        ESP_LOGW(TAG, "Already initialized");
        return true;
    }

    s_control_mutex = xSemaphoreCreateMutex();
    if (s_control_mutex == NULL) {
        ESP_LOGE(TAG, "Failed to create mutex");
        return false;
    }

    // Configure LEDC timer for SG90 PWM (50Hz, resolution = SG90_PWM_RESOLUTION_BITS)
    ledc_timer_config_t timer_conf = {
        .duty_resolution = SG90_PWM_RESOLUTION_BITS,
        .freq_hz = SG90_PWM_FREQ_HZ,
        .speed_mode = LEDC_LOW_SPEED_MODE,
        .timer_num = LEDC_TIMER_0,
        .clk_cfg = LEDC_AUTO_CLK,
        .deconfigure = false,
    };

    esp_err_t err = ledc_timer_config(&timer_conf);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "ledc_timer_config failed: %d", err);
        vSemaphoreDelete(s_control_mutex);
        s_control_mutex = NULL;
        return false;
    }

    // Configure LEDC channels 0..5 for pins PIN_PWM_1..PIN_PWM_6
    const int pwm_pins[6] = {PIN_PWM_1, PIN_PWM_2, PIN_PWM_3, PIN_PWM_4, PIN_PWM_5, PIN_PWM_6};
    for (int i = 0; i < 6; i++) {
        ledc_channel_config_t ch_conf = {
            .channel = (ledc_channel_t)i,
            .duty = 0,
            .gpio_num = pwm_pins[i],
            .speed_mode = LEDC_LOW_SPEED_MODE,
            .hpoint = 0,
            .timer_sel = LEDC_TIMER_0,
            .flags.output_invert = 0,
        };
        err = ledc_channel_config(&ch_conf);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "ledc_channel_config(ch=%d) failed: %d", i, err);
            return false;
        }
    }

    // Configure digital outputs (LEDs)
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << PIN_OUTPUT_1) | (1ULL << PIN_OUTPUT_2),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    err = gpio_config(&io_conf);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "gpio_config failed: %d", err);
        return false;
    }

    // Safe initial state
    s_outputs.pwm_ch1_us = 0;     // throttle ctrl -> 0
    s_outputs.pwm_ch2_us = 500;   // roll center
    s_outputs.pwm_ch3_us = 500;   // pitch center
    s_outputs.pwm_ch4_us = 500;   // yaw center
    s_outputs.pwm_ch5_us = 0;
    s_outputs.pwm_ch6_us = 0;
    s_outputs.dig_ch1_state = 0;
    s_outputs.dig_ch2_state = 0;

    s_armed = true;
    s_failsafe_active = false;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        apply_outputs_locked();
        xSemaphoreGive(s_control_mutex);
    }

    s_initialized = true;
    return true;
}

void control_update_from_sbus(const uint16_t channels[16])
{
    if (!s_initialized || channels == NULL) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;

    // If we were in failsafe lock, first valid frame should release it.
    if (s_failsafe_active) {
        s_failsafe_active = false;
    }

    // If disarmed: cut throttle in apply stage.

    // Mapping based on your earlier code:
    // - CH1 roll  (SBUS_CH_1)
    // - CH2 pitch (SBUS_CH_2)
    // - CH3 throttle (SBUS_CH_3)
    // - CH4 yaw (SBUS_CH_4)
    // - CH5 AUX1 -> dig_ch1_state
    // - CH6 AUX2 -> dig_ch2_state
    ESP_LOGI(TAG, "Channels: CH1=%u, CH2=%u, CH3=%u, CH4=%u, CH5=%u, CH6=%u,CH7=%u, CH8=%u, CH9=%u, CH10=%u, CH11=%u, CH12=%u, CH13=%u, CH14=%u, CH15=%u, CH16=%u",
        channels[SBUS_CH_1], channels[SBUS_CH_2], channels[SBUS_CH_3], channels[SBUS_CH_4], channels[SBUS_CH_5], channels[SBUS_CH_6], 
        channels[SBUS_CH_7], channels[SBUS_CH_8], channels[SBUS_CH_9], channels[SBUS_CH_10], channels[SBUS_CH_11], channels[SBUS_CH_12], 
        channels[SBUS_CH_13], channels[SBUS_CH_14], channels[SBUS_CH_15], channels[SBUS_CH_16]);
    uint16_t roll_ctrl = sbus_to_0_1000(channels[SBUS_CH_1]);
    uint16_t pitch_ctrl = sbus_to_0_1000(channels[SBUS_CH_2]);
    uint16_t throttle_ctrl = sbus_to_0_1000(channels[SBUS_CH_3]);
    uint16_t yaw_ctrl = sbus_to_0_1000(channels[SBUS_CH_4]);

    s_outputs.pwm_ch1_us = throttle_ctrl; // throttle
    s_outputs.pwm_ch2_us = roll_ctrl;
    s_outputs.pwm_ch3_us = pitch_ctrl;
    s_outputs.pwm_ch4_us = yaw_ctrl;

    // AUX -> digital states
    s_outputs.dig_ch1_state = (channels[SBUS_CH_5] > 1500) ? 1 : 0;
    s_outputs.dig_ch2_state = (channels[SBUS_CH_6] > 1500) ? 1 : 0;

    // AUX PWM (optional): store AUX CTRL scaled to 0..1000
    s_outputs.pwm_ch5_us = sbus_to_0_1000(channels[SBUS_CH_5]);
    s_outputs.pwm_ch6_us = sbus_to_0_1000(channels[SBUS_CH_6]);

    apply_outputs_locked();

    xSemaphoreGive(s_control_mutex);
}

void control_set_outputs(const control_outputs_t *outputs)
{
    if (!s_initialized || outputs == NULL) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;

    s_outputs = *outputs;

    // Clamp safety
    s_outputs.pwm_ch1_us = clamp_u16(s_outputs.pwm_ch1_us, 0, 1000);
    s_outputs.pwm_ch2_us = clamp_u16(s_outputs.pwm_ch2_us, 0, 1000);
    s_outputs.pwm_ch3_us = clamp_u16(s_outputs.pwm_ch3_us, 0, 1000);
    s_outputs.pwm_ch4_us = clamp_u16(s_outputs.pwm_ch4_us, 0, 1000);
    s_outputs.pwm_ch5_us = clamp_u16(s_outputs.pwm_ch5_us, 0, 1000);
    s_outputs.pwm_ch6_us = clamp_u16(s_outputs.pwm_ch6_us, 0, 1000);
    s_outputs.dig_ch1_state = outputs->dig_ch1_state ? 1 : 0;
    s_outputs.dig_ch2_state = outputs->dig_ch2_state ? 1 : 0;

    // If disarmed or failsafe active, enforce throttle cut in apply stage.

    apply_outputs_locked();

    xSemaphoreGive(s_control_mutex);
}

void control_get_outputs(control_outputs_t *outputs)
{
    if (!s_initialized || outputs == NULL) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;
    memcpy(outputs, &s_outputs, sizeof(control_outputs_t));
    xSemaphoreGive(s_control_mutex);
}

void control_arm(void)
{
    if (!s_initialized) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;

    if (!s_armed) {
        s_armed = true;
        s_failsafe_active = false;

        // Keep centers
        s_outputs.pwm_ch2_us = 500;
        s_outputs.pwm_ch3_us = 500;
        s_outputs.pwm_ch4_us = 500;
        s_outputs.pwm_ch1_us = 0;

        apply_outputs_locked();
        ESP_LOGI(TAG, "System ARMED");
    }

    xSemaphoreGive(s_control_mutex);
}

void control_disarm(void)
{
    if (!s_initialized) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;

    if (s_armed) {
        s_armed = false;
        // throttle cut in apply stage
        s_outputs.pwm_ch1_us = 0;
        apply_outputs_locked();
        ESP_LOGI(TAG, "System DISARMED");
    }

    xSemaphoreGive(s_control_mutex);
}

bool control_is_armed(void)
{
    if (!s_initialized || s_control_mutex == NULL) return false;

    bool armed = false;
    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        armed = s_armed;
        xSemaphoreGive(s_control_mutex);
    }
    return armed;
}

void control_failsafe(void)
{
    if (!s_initialized) return;

    if (xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return;

    s_failsafe_active = true;
    s_armed = false;

    // Failsafe policy: cut throttle, center attitude, turn LEDs on.
    s_outputs.pwm_ch1_us = 0;
    s_outputs.pwm_ch2_us = 500;
    s_outputs.pwm_ch3_us = 500;
    s_outputs.pwm_ch4_us = 500;
    s_outputs.dig_ch1_state = 1;
    s_outputs.dig_ch2_state = 1;

    apply_outputs_locked();
    ESP_LOGW(TAG, "FAILSAFE activated");

    xSemaphoreGive(s_control_mutex);
}

void control_deinit(void)
{
    if (!s_initialized) return;

    if (s_control_mutex != NULL) {
        xSemaphoreTake(s_control_mutex, pdMS_TO_TICKS(100));
        xSemaphoreGive(s_control_mutex);
        vSemaphoreDelete(s_control_mutex);
        s_control_mutex = NULL;
    }

    // Reset state
    s_armed = false;
    s_failsafe_active = false;
    memset(&s_outputs, 0, sizeof(s_outputs));
    s_initialized = false;

    // Stop PWM outputs: set duty 0
    for (int i = 0; i < 6; i++) {
        set_pwm_duty((ledc_channel_t)i, 0);
    }



    gpio_set_level(PIN_OUTPUT_1, 0);
    gpio_set_level(PIN_OUTPUT_2, 0);

    ESP_LOGI(TAG, "Control system deinitialized");
}

